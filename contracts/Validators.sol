...
contract Validators is Params {
        ...
        onlyAdmin
    {
        ...
        emit LogChangeDec(multi_, divisor_);
    }
    ...
    function stake(
        ...
        emit LogAddStake(msg.sender, msg.value);
    }
    ...
    function restake() external payable onlyInitialized returns (bool) {
        address validator = msg.sender;
        // validatorInfo[validator].coins > 0 when validator is jailed and not
        // withdraw his staking.
        uint256 staking = msg.value.add(validatorInfo[validator].coins);
        require(
            validatorInfo[validator].status == Status.Jailed ||
                validatorInfo[validator].status == Status.Unstaked,
            "Invalid status, can't restake"
        );
        if (validatorInfo[validator].status == Status.Unstaked) {
            require(
                block.number >=
                    RestakingLockPeriodUnstaked +
                        validatorInfo[validator].removedBlock,
                "You haven't wait enough blocks to restake(unstaked)"
            );
        }
        if (validatorInfo[validator].status == Status.Jailed) {
            require(
                block.number >=
                    RestakingLockPeriodJailed +
                        validatorInfo[validator].removedBlock,
                "You haven't wait enough blocks to restake(jailed)"
            );
        }
        require(staking >= MinimalStakingCoin, "Staking coins not enough");
    ...
    function withdrawStaking() external returns (bool) {
        ...
        if (validatorInfo[validator].status == Status.Unstaking) {
            validatorInfo[validator].status = Status.Unstaked;
        }

        validatorInfo[validator].feeAddr.transfer(staking);
    ...
    function safeTransferHSCT(address to, uint256 amount) internal {
        if (amount > hsctToken.balanceOf(to)) {
            amount = hsctToken.balanceOf(to);
        }
